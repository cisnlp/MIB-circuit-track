...
class FQwen2Attention(Qwen2Attention):
    ...
    def forward(
        ...
        key_states = self._apply_headwise_linear(
            k_hidden_states, self.k_proj, self.num_key_value_heads
        )
        value_states = self._apply_headwise_linear(
            v_hidden_states, self.v_proj, self.num_key_value_heads
        )
    ...
    def set_attn_mask_value(self, from_idx, head_idx, qkv, value):
        if qkv == "q":
            ...
            if head_idx >= self.num_kv_heads:
                ...
                )  # Map full attention head -> KV group
                if group_size == 0:
                    raise ValueError(
                        "Invalid GQA configuration: num_heads < num_kv_heads."
                    )
                kv_idx = head_idx // group_size
            else:
                kv_idx = head_idx

            if kv_idx >= self.num_kv_heads:
                raise IndexError(
                    f"Computed kv_idx={kv_idx} out of range for num_kv_heads={self.num_kv_heads}"
                )

            if qkv == "k":
                old_value = self.k_read_log_alphas[from_idx, kv_idx].detach().item()
                self.k_read_log_alphas[from_idx, kv_idx] = value
            elif qkv == "v":
                old_value = self.v_read_log_alphas[from_idx, kv_idx].detach().item()
                self.v_read_log_alphas[from_idx, kv_idx] = value
            else:
        ...
        for layer_id in range(self.num_layers):
            ...
            for from_idx in range(attn_q_edges.shape[0]):
                ...
                for head_no in range(attn_q_edges.shape[1]):
                    if attn_q_edges[from_idx, head_no] == 1:
                        to_idx = (
                            layer_id * (1 + self.num_heads + 2 * self.num_kv_heads)
                            + head_no
                        )
                        edges.append(
                            (
            ...
            for from_idx in range(mlp_edges.shape[0]):
                ...
                if mlp_edges[from_idx] == 1:
                    to_idx = (layer_id + 1) * (
                        1 + self.num_heads + 2 * self.num_kv_heads
                    ) - 1
                    edges.append(
                        (
                            writer_idx_to_name(
