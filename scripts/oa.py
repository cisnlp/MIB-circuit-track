...
def load_interpbench_model():
    ...
    model = UnifiedTransformer(
        cfg, state_dict=torch.load(hf_model, map_location="cuda"), from_cfg=True
    )
    return model
...
class OptimalAblationFinder:
    ...
    def compute_means(self, dataloader: DataLoader) -> Dict[str, torch.Tensor]:
        mean_activations = {}
        ...
        for component in self.target_components:
            ...
            if "attn" in component:  # attention
                zeros = torch.zeros(self.model.cfg.n_heads, self.model.cfg.d_model).to(
                    "cuda"
                )
            else:
        ...
        n_batches = 0
        for step in tqdm(
            range(self.num_steps), desc="Example:", total=(self.num_steps)
        ):
            # Get batch
            try:
            ...
            for component in self.target_components:
                ...
                with torch.no_grad():
                    with self.model.trace(batch["input_ids"]):
    ...
    def compute_optimal_ablation(
        self, dataloader: DataLoader, mean_activations: Dict[str, torch.Tensor]
        ...
        for component in self.target_components:
            ablation_values[component] = nn.Parameter(
                mean_activations[component], requires_grad=True
            )
            # Get the component's output size from a forward pass
            # layer = self.model.transformer.h[int(component.split('.')[2])]
            # submodule = eval(f"self.model.{component}")
        ...
        for step in tqdm(range(self.num_steps), total=self.num_steps, desc="Step"):
            ...
            for component in self.target_components:
                ...
                with self.model.trace(batch["input_ids"]):
                    ...
                    if "attn" in component:
                        ablated_output = (
                            ablation_values[component]
                            .expand(
                                target.output.shape[0], target.output.shape[1], -1, -1
                            )
                            .to(self.dtype)
                        )
                    else:
                        ablated_output = (
                            ablation_values[component]
                            .expand(target.output.shape[0], target.output.shape[1], -1)
                            .to(self.dtype)
                        )
            ...
            if step % self.check_every == 0:
                print(
                    f"Step {step}, Average Loss: {total_loss / len(self.target_components):.4f}"
                )
                if total_loss / len(self.target_components) < self.prev_best_loss:
                    ...
                    self.not_improved += 1
                    print(
                        f"Loss not improved for {self.not_improved} evaluations (/ {self.patience})"
                    )
        ...
        self.optimal_values = {
            component: value.detach() for component, value in ablation_values.items()
        }

        return self.optimal_values

    def apply_optimal_ablation(
        self, component: str, permanent: bool = False
    ...
    for model_name in models:
        for task in tasks:
            ...
            if model_name == "gpt2":
                ...
                model = UnifiedTransformer(
                    "Qwen/Qwen2.5-0.5B", attn_implementation="eager"
                )
            ...
            model.cfg.ungroup_grouped_query_attention = (
                True if model_name in ("gemma2", "qwen2.5", "llama3") else False
            )
            tokenizer = model.tokenizer

            split = "train"
            dataset_name = (
                "mib-bench/copycolors_mcqa" if task == "mcqa" else "mib-bench/ioi"
            )
            ...
            optimal_values = finder.compute_optimal_ablation(
                dataloader, mean_activations
            )
