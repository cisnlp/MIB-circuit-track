...
def get_metric(
    ...
    if metric_name == "kl_divergence" or metric_name == "kl":
        ...
        if "greater-than" in task:
            assert (
                tokenizer is not None or model is not None
            ...
            logit_diff_fn = partial(
                logit_diff_greater_than,
                indices=get_year_indices(model_name, tokenizer),
...
def normalized_logit(
    ...
    good_logits = torch.gather(circuit_outputs, -1, labels.to(circuit_outputs.device))[
        :, 0
    ]
...
def topk_accuracy(
    ...
    topk_acc = torch.any(
        predictions.eq(labels.expand_as(predictions)), dim=-1
    ).float()  # Check if any of the top-k predictions match the label
    if loss:
        topk_acc = 1 - topk_acc
    if mean:
...
def get_logit_positions(
    logits: torch.Tensor, input_length: torch.Tensor, position: int = -1
) -> torch.Tensor:
...
def divergence(
    ...
    if divergence_type == "kl":
        results = kl_div(
            circuit_probs.log(), clean_probs.log(), log_target=True, reduction="none"
        ).mean(-1)
    ...
    if model_name not in gt_indices_dict:
        if "gpt2" in model_name:
            ...
            gt_indices_dict[model_name] = torch.tensor(
                [
                    tokenizer(f"{year:02d}", add_special_tokens=False).input_ids[0]
            ...
            three_digit_indices = torch.tensor(
                [
                    tokenizer(f"{digit:03d}", add_special_tokens=False).input_ids[0]
                    for digit in range(1000)
                ]
            )
            single_digit_indices = torch.tensor(
                [
                    tokenizer(f"{digit}", add_special_tokens=False).input_ids[-1]
...
def logit_diff_greater_than(
    ...
    labels: torch.Tensor,
    model_name: str,
    indices: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
    mean: bool = True,
    prob: bool = False,
    loss: bool = False,
):
    ...
    if "gpt2" in model_name:
        ...
        for decade_probs, year_probs, (decade, year) in zip(
            probs_dec[:, decade_indices], probs_yr[:, digit_indices], labels
        ):
        ...
        for decade_probs, year_probs, (decade, year) in zip(
            probs_dec[:, digit_indices], probs_yr[:, digit_indices], labels
        ):
            decade_prob = decade_probs[decade]
            good_decade_probs = decade_probs[decade + 1 :]
            bad_decade_probs = decade_probs[:decade]
...
def sequence_logprob(
    ...
    labels = torch.tensor(
        padded_labels, dtype=torch.long, device=circuit_outputs.device
    )[:, 0, :]
    _ = circuit_outputs[:, -labels.shape[-1] :, :]
