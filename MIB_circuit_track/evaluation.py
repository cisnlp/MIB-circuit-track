...
def evaluate_area_under_curve(
    ...
    intervention: Literal[
        "patching", "zero", "mean", "mean-positional", "optimal"
    ] = "patching",
    ...
    if level == "neuron":
        assert (
            graph.neurons_scores is not None
        ), "Neuron scores must be present for neuron-level evaluation"
        n_scored_items = (~torch.isnan(graph.neurons_scores)).sum().item()
    elif level == "node":
        assert (
            graph.nodes_scores is not None
        ), "Node scores must be present for node-level evaluation"
    ...
    for pct in percentages:
        ...
        if apply_greedy:
            assert (
                level == "edge"
            ), "Greedy application only supported for edge-level evaluation"
        ...
        if no_normalize:
            ...
            faithfulness = (ablated_score - corrupted_score) / (
...
def evaluate_area_under_roc(reference: Graph, hypothesis: Graph, by_node: bool = False):
    ...
    if by_node:
        ref_objs = reference.nodes  # noqa: F841
        hyp_objs = hypothesis.nodes
    else:
        ref_objs = reference.edges
        hyp_objs = hypothesis.edges  # noqa: F841

    num_objs = len(ref_objs.values())
    for pct in (0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1):
