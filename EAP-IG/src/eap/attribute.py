...
def _get_importance_ifr(
    acts, prev_acts,
    style:Literal["norm", "dot", "own"]="norm", order=1, clamp_min=True, clamp_max=True,
):
    if style=="norm":
        assert clamp_min==clamp_max
        proximity = - torch.linalg.vector_norm(
            prev_acts - acts, ord=order, dim=-1
        ) + torch.linalg.vector_norm(
            acts, ord=order, dim=-1
        )
        if clamp_min:
            proximity = torch.clamp(proximity, min=0)
    elif style=="dot":
        proximity = einops.einsum(
            prev_acts, acts,
            "batch pos fw bw hidden, batch pos fw bw hidden -> batch pos fw bw"
        )
        if clamp_max:
            sqnorm_act = einops.reduce(
                proximity, "batch pos fw bw -> batch pos 1 bw 1", "sum"
            )#more efficient way to compute squared norm of act, only involves sums
            proximity = torch.clamp(proximity, max=sqnorm_act)
        if clamp_min:
            proximity = torch.clamp(proximity, min=0)
    elif style=="own":
        proximity = torch.linalg.vector_norm(prev_acts, ord=order, dim=-1)
        shape = list(proximity.shape)
        shape[3] = acts.shape[3] #expand "backward" dim
        proximity = proximity.broadcast_to(shape)
    else:
...
def get_scores_information_flow_routes(
    model: HookedTransformer, graph: Graph, dataloader: DataLoader, quiet=False, **kwargs
) -> torch.Tensor:
    ...
    Args:
        ...
        metric (Callable[[Tensor], Tensor]): the metric to attribute with respect to
        quiet (bool, optional): whether to silence tqdm. Defaults to False.
        **kwargs: combine to your favourite variant of the importance score
    ...
    def make_hooks(
        n_pos: int, input_lengths: torch.Tensor, **kwargs
        ...
        def input_hook(
            prev_index, bwd_index, input_lengths, activations, hook,
            **kwargs
            ):
            acts = activations.detach()
            try:
                ...
                importance = _get_importance_ifr(
                    acts, unsqueezed_output_activations[:,:,:prev_index], **kwargs
                )
        ...
        for layer in range(graph.cfg['n_layers']):
            ...
            for i, letter in enumerate('qkv'):
                ...
                hooks.append((
                    node.qkv_inputs[i],
                    partial(input_hook, prev_index, bwd_index, input_lengths, **kwargs)
                ))
            ...
            prev_index = graph.prev_index(node)
            hooks.append((node.out_hook, partial(output_hook, fwd_index)))
            hooks.append((node.in_hook, partial(input_hook, prev_index, bwd_index, input_lengths, **kwargs)))
    ...
    for clean, _, _ in dataloader:
        ...
        hooks = make_hooks(n_pos, input_lengths, **kwargs)
        with torch.inference_mode():
...
def attribute(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric: Callable[[Tensor], Tensor], 
    ...
    if method == 'EAP':
        ...
        scores = get_scores_ig_activations(model, graph, dataloader, metric, steps=ig_steps, intervention=intervention, 
                                           intervention_dataloader=intervention_dataloader, optimal_ablation_path=optimal_ablation_path, quiet=quiet)
    elif method[:23] == 'information-flow-routes':
        #pattern: information-flow-routes-norm-minmax1 (this is the default)
        if len(method)>23:
            args_list = method[24:].split('-')
            order_char = args_list[1][-1]
            kwargs = {
                "style": args_list[0],
                "clamp_min": "min" in args_list[1],
                "clamp_max": "max" in args_list[1],
                "order": int(order_char) if order_char.isnumeric() else None,
            }
            print(kwargs)
        else:#default
            kwargs = {}
        scores = get_scores_information_flow_routes(model, graph, dataloader, quiet=quiet, **kwargs)
    elif method == 'exact':
